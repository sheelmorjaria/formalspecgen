...
from .canonical_draft import (
    canonical_draft_c,
    canonical_draft_cpp,
    canonical_draft_java,
    canonical_draft_rust,
)
...
SESSION_VERSION = 1
CLI_COMMAND_PLUGIN_ABI = "formalspecgen.cli-command-plugin.v1"


class CommandError(RuntimeError):
    """A plugin command failure with a stable process exit code."""
...
def register_plugin(
    ...
    """Register a validated external command plugin with an argparse parser."""
    if plugin.abi_id != CLI_COMMAND_PLUGIN_ABI:
        raise ValueError(
            f"unsupported command plugin ABI {plugin.abi_id!r}; "
            f"expected {CLI_COMMAND_PLUGIN_ABI!r}"
        )
    known = set(existing) | set(subparsers.choices)
    registered: list[str] = []
    for declaration in plugin.declarations:
        if declaration.command_name in known:
            raise ValueError(f"duplicate CLI command {declaration.command_name!r}")
        if declaration.human_only and declaration.mcp_exposed:
            raise ValueError(
                f"human-only command {declaration.command_name!r} cannot be MCP-exposed"
            )
        command = subparsers.add_parser(
            declaration.command_name,
            help=declaration.capability_name.replace("_", " "),
        )
...
class TerminalUI:
    def __init__(
        self, console: Console | None = None, ask: Callable[[str], str] | None = None
    ):
    ...
    def event(self, event: dict[str, Any]) -> None:
        ...
        if kind == "progress":
            self.console.print(
                f"[cyan]•[/cyan] {event.get('message', event.get('stage', 'Working'))}"
            )
        elif kind == "spec_warning":
            self.console.print(
                f"[yellow]⚠ line {event.get('line', 0)}:[/yellow] {event.get('message', '')}"
            )
            ...
            if explanation:
                self.console.print(
                    Panel(str(explanation), title="Explanation", border_style="yellow")
                )
        elif kind == "attempt_complete":
            style = "green" if event.get("status") == "VERIFIED" else "red"
            self.console.print(
                f"[{style}]Attempt {event.get('attempt')}: {event.get('status')}[/{style}]"
            )
    ...
    def clarify(
        ...
        if state.get("requirement") != requirement or not state.get("questions"):
            questions, _, _ = extract_ambiguities(
                requirement, _chat_fn(provider), model
            )
...
def _finish_canonical_draft(
    ...
    ui.console.print(
        Panel(
            ...
            title="Reviewed domain contract",
            border_style="green",
        )
    )
    return 0
...
def command_draft(
    ...
    try:
        enriched = (
            ui.clarify(requirement, args.provider, args.model, state, store)
...
def _finish_language_draft(
    ...
    for warning in result.get("warnings", []):
        ui.console.print(
            f"[yellow]⚠ line {warning.get('line', 0)}:[/yellow] {warning.get('message', '')}"
        )
...
def command_implement(args: argparse.Namespace, ui: TerminalUI) -> int:
    ...
    if suffix not in {".java", ".jml", ".rs", ".c", ".cpp", ".cc", ".cxx"}:
        ui.console.print(
            f"[bold red]Unsupported implementation source: {suffix or '<none>'}[/bold red]"
        )
        return 2
    ...
    if dependency:
        ...
        if dependency not in allowed.get(suffix, set()):
            ui.console.print(
                f"[bold red]Dependency {dependency!r} cannot fill {suffix} adapters[/bold red]"
            )
            return 2
        dma_profile = None
    ...
    try:
        result = run_implementation_loop(
            ...
            resample_budget=args.resample_budget,
            feedback_budget=args.feedback_budget,
            accepted_passes=args.accept_pass,
            clarifications=args.clarifications or "",
        ...
        ui.console.print(
            f"[bold red]Implementation failed:[/bold red] {escape(str(exc))}"
        )
        return 2
    ...
    if parallel_wrapper is not None:
        if suffix != ".rs" or parallel_wrapper != "rayon":
            result = {
                **result,
                "final_status": "UNSUPPORTED_BOUNDARY",
                "claim": "NO_PROOF",
            ...
            from .parallel_wrapper import (
                check_rayon_syntax,
                parallel_partition_gate,
                render_rayon_wrapper,
            )
            ...
            try:
                ...
                partition = {
                    "status": "FAIL",
                    "claim": "NO_PROOF",
                    "code": "unsupported_kernel_boundary",
                    "message": str(exc),
                }
            else:
                ...
                partition = parallel_partition_gate(
                    ...
                    wrapped,
                    kernel_name,
                    kernel_deductive_proof=(
                        result.get("final_status") == "VERIFIED"
                        and result.get("claim")
                        in {"DEDUCTIVE_PROOF", "SOURCE_MODEL_REFINEMENT"}
                    ),
            ...
            if partition["status"] == "VERIFIED":
                destination = Path(
                    getattr(args, "parallel_out", None)
                    or Path(args.stub).with_name(Path(args.stub).stem + "_parallel.rs")
                )
                destination.write_text(wrapped, encoding="utf-8")
                result.update(
                    {
                        "final_status": "PARALLEL_PARTITION_VERIFIED",
                ...
                result.update(
                    {
                        "final_status": "PARALLEL_PARTITION_FAILED",
                        "claim": "NO_PROOF",
                        "partition_safety_proved": False,
                        "parallel_scheduler_proved": False,
                    }
                )
    _write_json(result, args.json, ui.console)
    return (
        ...
        in {
            "VERIFIED",
            "STATIC_CHECKED",
            "STATIC_CHECKED_RUNTIME_TESTED",
            "COMPILED_LINTED",
            "LOCK_DISCIPLINE_VERIFIED",
            "CONCURRENT_LINEARIZABILITY_VERIFIED",
            "PARALLEL_PARTITION_VERIFIED",
            "ASYNC_STATIC_CHECKED",
        }
        else 1
    )
...
def command_discover_algorithms(args: argparse.Namespace, ui: TerminalUI) -> int:
    ...
    selected = (
        ...
        else [item.strip() for item in args.strategies.split(",")]
    )
...
def command_remediate(args: argparse.Namespace, ui: TerminalUI) -> int:
    ...
    result = remediate(
        args.target, args.report, args.out_dir, provider=args.provider, model=args.model
    )
    _write_json(
        result,
        args.json or str(Path(args.out_dir) / "remediation_verdict.json"),
        ui.console,
    )
    ui.console.print(
        f"Status: {result['status']}\nClaim: {result.get('claim', 'NO_PROOF')}"
...
def command_correct_behavior(args: argparse.Namespace, ui: TerminalUI) -> int:
    ...
    _write_json(
        result,
        args.json or str(Path(args.out_dir) / "correction_verdict.json"),
        ui.console,
    )
    ui.console.print(
        f"Status: {result['status']}\nClaim: {result.get('claim', 'NO_PROOF')}"
    ...
    return (
        0
        if result["status"]
        in {"BEHAVIOR_CORRECTION_VERIFIED", "CAPACITY_BOUND_CANDIDATE_GENERATED"}
        else 1
    )
...
def command_architecture(args: argparse.Namespace, ui: TerminalUI) -> int:
    ...
    ui.console.print(
        Panel(
            ...
            border_style="green" if status == "VERIFIED" else "red",
        )
    )
    if args.emit_tla and result.get("tla"):
        Path(args.emit_tla).write_text(result["tla"], encoding="utf-8")
        Path(args.emit_tla).with_suffix(".cfg").write_text(
            result["cfg"], encoding="utf-8"
        )
    if args.json:
        _write_json(result, args.json, ui.console)
...
def command_design_system(args: argparse.Namespace, ui: TerminalUI) -> int:
    ...
    ui.console.print(
        f"[bold cyan]Designing architecture via {args.provider}…[/bold cyan]"
    )
    try:
        ...
        ui.console.print(
            f"[bold red]Architecture generation failed:[/bold red] {escape(str(exc))}"
        )
        return 1
    ...
    destination.write_text(
        json.dumps(result["architecture"], indent=2) + "\n", encoding="utf-8"
    )
    ...
    if args.json:
        ...
        for item in report["capabilities"]:
            ...
            ceiling = (
                ...
                else f"judge_pending:{item['judge_pending']}"
            )
            table.add_row(item["name"], item["status"], ceiling)
        ui.console.print(table)
        ui.console.print(
            "[dim]Readiness is not verification evidence; doctor always reports claim=NO_PROOF.[/dim]"
        )
        for item in report["capabilities"]:
            if item.get("message"):
                ui.console.print(
                    f"[yellow]{item['name']}:[/yellow] {escape(item['message'])}"
                )
        ...
        for domain in report["domains"]:
            domains.add_row(
                domain["name"], domain["maturity"], domain["evidence_ceiling"]
            )
        ...
        for lane in report["lanes"]:
            lanes.add_row(
                lane["lane"],
                str(lane["current_step"]),
                lane["maturity"],
                lane["status"],
                ", ".join(lane["claims_locked"]) or "none",
            )
...
def command_domain(
    ...
    try:
        ...
        if (
            draft.get("idea") != idea
            or not draft.get("questions")
            or int(draft.get("schema_version", 1)) != schema_version
        ):
            questions, _, _ = elicit_domain_questions(
                idea, _chat_fn(args.provider), args.model
            )
            draft = {
                "idea": idea,
        ...
        for index, question in enumerate(draft["questions"], 1):
            ...
            store.save(state)
        compiler = (
            compile_domain_spec_v2 if schema_version == 2 else compile_domain_spec
        )
        compiler_chat = _chat_fn(
            args.provider,
            json_schema=DomainSpecV2.model_json_schema()
            if schema_version == 2
            else None,
        )
        compiler_kwargs = (
            {
                "progress": lambda message: ui.console.print(
        ...
        canonical = (
            root / "domains" / "v2" / f"{spec.module_name}.json"
            if schema_version == 2
            else root / "domains" / f"{spec.module_name}.yaml"
        )
        ...
        if candidate.exists() and not args.force:
            raise FileExistsError(
                f"refusing to overwrite candidate {candidate}; pass --force"
            )
        candidate.write_text(yaml_text, encoding="utf-8")
        outputs = (
            []
            if schema_version == 2
            else scaffold_domain(
                candidate,
                project_root=root,
                force=args.force,
        ...
        if schema_version == 2:
            ui.console.print(
                "[bold magenta]Running deterministic schema and TLC validation…[/bold magenta]"
            )
        ...
        ui.console.print(
            f"[bold red]Domain generation failed:[/bold red] {escape(str(exc))}"
        )
        return 2
    state["domain_draft"] = {}
    store.save(state)
    ...
    if schema_version == 1:
        ui.console.print(
            "[yellow]Human review is required for extractor and renderer TODOs.[/yellow]"
        )
    else:
        ui.console.print(
            Panel(
...
def command_manage_trust(args: argparse.Namespace, ui: TerminalUI) -> int:
    ...
    if result is not None:
        ui.console.print(
            f"{result['status']}: {result['key_id']} ({result['registry']})"
        )
    for item in trust.list_trusted_keys(args.registry):
        ui.console.print(
            f"  trusted: {item['key_id']}  (from {item.get('source', '?')})"
        )
    return 0
...
def command_verify_unbounded(args: argparse.Namespace, ui: TerminalUI) -> int:
    ...
    result = verify_unbounded(
        args.source, invariant=args.invariant, provider=args.provider
    )
    if args.json_out:
        _write_json(result, args.json_out, ui.console)
...
def command_verify_distributed(args: argparse.Namespace, ui: TerminalUI) -> int:
    ...
    fields = [item.strip() for item in args.message_fields.split(",") if item.strip()]
    result = verify_distributed(args.domain, faults=faults, message_fields=fields)
    if args.json_out:
        _write_json(result, args.json_out, ui.console)
...
def command_verify_lockfree(args: argparse.Namespace, ui: TerminalUI) -> int:
    ...
    if args.json_out:
        _write_json(result, args.json_out, ui.console)
    style = "green" if result["status"] == "LOCK_FREE_LINEARIZABILITY_PROVED" else "red"
    detail = result.get("message") or result.get("scope", "concurrent_interleaving_bmc")
    ui.console.print(f"[{style}]{result['status']}[/{style}] — {detail}")
...
def command_macro_dictionary(args: argparse.Namespace, ui: TerminalUI) -> int:
    ...
    from .macro_semantics import (
        load_macro_dictionary,
    ...
    if args.synthesize:
        ...
        if loaded.get("status") != "MACRO_DICTIONARY_LOADED":
            ...
            result = translate_macros(
                args.source, loaded["dictionary"], provider=args.provider
            )
    else:
        result = load_macro_dictionary(args.dictionary)
    if args.json_out:
        _write_json(result, args.json_out, ui.console)
    proved = result.get("status") in {
        "MACRO_DICTIONARY_LOADED",
        "MACROS_TRANSLATED",
        "V2_SYNTHESIZED_FROM_MACROS",
    }
    style = "green" if proved else "red"
    detail = result.get("message") or result.get("status", "")
    ui.console.print(f"[{style}]{result.get('status', 'FAILED')}[/{style}] — {detail}")
...
def command_promote_domain(args: argparse.Namespace, ui: TerminalUI) -> int:
    ...
    try:
        ...
        ui.console.print(
            f"[bold red]Domain promotion failed:[/bold red] {escape(str(exc))}"
        )
        return 2
    ...
    if int(requested_schema) == 2:
        ...
        try:
            ...
            ui.console.print(
                f"[bold red]V2 domain promotion failed:[/bold red] {escape(str(exc))}"
            )
            return 2
        ui.console.print(
    ...
    try:
        ...
        if spec.module_name != name:
            raise ValueError(
                f"candidate declares module {spec.module_name!r}, expected {name!r}"
            )
        if (
            canonical.exists()
            and load_spec(canonical).review_status == "reviewed"
            and not (args.replace_reviewed_domain)
        ):
            raise PermissionError(
        ...
        if missing:
            raise FileNotFoundError(
                "promotion artifacts are missing: " + ", ".join(missing)
            )
        forbidden = (
            "TODO",
            "plugin is scaffolded",
        ...
        if blocked:
            raise ValueError(
                "adapter/renderer remains an unreviewed fail-closed stub: "
                + ", ".join(blocked)
            )
        reviewed = spec.model_copy(
            update={"review_status": "reviewed", "schema_version": 1}
        )
        ...
        return 2
    ui.console.print(
        f"[green]Promoted reviewed domain {name}[/green]\n  [path]{canonical}[/path]"
    )
    return 0
...
def command_compose(args: argparse.Namespace, ui: TerminalUI) -> int:
    ...
    try:
        ...
        ui.console.print(
            f"[bold red]Composition artifact unreadable:[/bold red] {escape(str(exc))}"
        )
        return 2
    if getattr(args, "lang", "java") != "java":
        ...
        ui.console.print(
            Panel(
                ...
                title="Polyglot composition verification",
                border_style=style,
            )
        )
    ...
    verdict = composition_render.verify_composition(
        value,
        args.v2_dir,
        run_esc=not args.no_esc,
        actors=getattr(args, "actors", None).split(",")
        if getattr(args, "actors", None)
        else None,
    )
    if args.out_dir and verdict.get("files"):
        destination = Path(args.out_dir)
    ...
    if verdict.get("disclaimer"):
        ui.console.print(verdict["disclaimer"], style="dim")
    return (
        0 if verdict["status"] in {"COMPOSITION_VERIFIED", "COMPOSITION_CHECKED"} else 1
    )
...
def command_reverify(args: argparse.Namespace, ui: TerminalUI) -> int:
    ...
    ui.console.print(
        Panel(
            ...
            title="Composition re-verification",
            border_style=style,
        )
    )
...
def command_system(args: argparse.Namespace, ui: TerminalUI) -> int:
    ...
    style = (
        ...
        if verdict["status"]
        in {
            "SYSTEM_SYNTHESIS_VERIFIED",
            "SYSTEM_REFACTOR_VERIFIED",
            "SYSTEM_CORRECTION_VERIFIED",
        }
        else "red"
    )
    ui.console.print(
...
def command_unified_system(args: argparse.Namespace, ui: TerminalUI) -> int:
    if args.lang != "java":
        ...
        try:
            ...
            ui.console.print(
                f"[bold red]Artifact unreadable:[/bold red] {escape(str(exc))}"
            )
            return 2
        verdict = verify_polyglot_composition(
...
def build_parser(
    ...
    parser.add_argument(
        "--version", action="version", version=f"formalspecgen {__version__}"
    )
    ...
    draft = sub.add_parser(
        "draft", parents=[common], help="clarify NL and draft checked JML"
    )
    ...
    implement.add_argument(
        "--assurance-level",
        choices=["critical", "standard", "lightweight"],
        default="critical",
    )
    implement.add_argument(
        "--method-proof-only",
    ...
    implement.add_argument(
        "--abstraction",
        choices=["atomic_operations", "lock_protocol"],
    ...
    verify_refactor.add_argument(
        "baseline", help="previously trusted Java/JML revision"
    )
    verify_refactor.add_argument(
    ...
    verify_refactor.add_argument(
        "--signing-key", help="GPG key ID for a detached verdict signature"
    )
    ...
    discover.add_argument(
        "--strategies", default="all", help="all or comma-separated strategy names"
    )
    discover.add_argument("--max-workers", type=int, default=3)
    discover.add_argument("--json")

    security = sub.add_parser(
        "assess-security", help="run formal CWE mapping and SAST assessment"
    )
    ...
    security_inspect = sub.add_parser(
        "security-inspect", help="inspect code for formal and SAST findings"
    )
    security_inspect.add_argument("source")
    security_inspect.add_argument("--json")
    security_exploit = sub.add_parser(
        "security-exploit", help="generate safe local JUnit PoC templates"
    )
    ...
    remediation = sub.add_parser(
        "remediate", parents=[common], help="generate a verified defensive patch copy"
    )
    ...
    inspect = sub.add_parser(
        "inspect",
        help="deterministically inspect one Java class for refactoring signals",
    )
    ...
    apply_refactor = sub.add_parser(
        "apply-refactor",
        help="apply a hash-bound deterministic Java refactoring profile",
    )
    apply_refactor.add_argument("source", help="baseline Java/JML source")
    apply_refactor.add_argument(
        "--inspection", help="hash-bound inspect JSON evidence (Java lanes)"
    )
    apply_refactor.add_argument(
        "--pattern",
        choices=[
    ...
    apply_refactor.add_argument(
        "--method", required=True, help="inspected long method name"
    )
    apply_refactor.add_argument(
        "--out", required=True, help="same-named refactored Java path"
    )
    apply_refactor.add_argument("--json", help="combined transformation/proof evidence")

    architecture = sub.add_parser(
        "architecture", help="render typed IR and run bounded TLC"
    )
    ...
    sign_cmd = sub.add_parser(
        "sign-artifact", help="create a detached GPG signature over an artifact"
    )
    sign_cmd.add_argument("artifact")
    sign_cmd.add_argument("--key", required=True, help="local-user key id or email")
    heap = sub.add_parser(
        "verify-heap", help="unbounded heap-shape verification (ghost predicates)"
    )
    heap.add_argument(
        "source",
    ...
    hal.add_argument(
        "source",
        help="C .c/.h HAL source (bitfield register "
    ...
    macros.add_argument(
        "dictionary", help="macros.json mapping macro names to V2 categories"
    )
    macros.add_argument(
        "--source",
        default=None,
        help="C .c/.h source whose macro invocations are translated",
    )
    macros.add_argument(
        "--synthesize",
        action="store_true",
        help="run the full macros-to-V2 chain (extraction + real-Z3 model proof)",
    )
    macros.add_argument(
        "--no-verify", action="store_true", help="skip the Z3 model proof"
    )
    ...
    lockfree.add_argument(
        "source",
        help="C .c pthread source (SPSC ring: buf + head/tail, one store each)",
    )
    lockfree.add_argument("--json", dest="json_out", default=None)
    dist = sub.add_parser(
        "verify-distributed", help="safety under injected network faults"
    )
    dist.add_argument("domain", help="async_message_passing V2 domain (yaml/json)")
    dist.add_argument(
        "--faults",
        default="message_loss,duplication,reordering",
    ...
    lin.add_argument(
        "--domain", required=True, help="V2 lock-protocol domain (yaml/json)"
    )
    lin.add_argument("--json", dest="json_out", default=None)
    unbounded = sub.add_parser(
        "verify-unbounded", help="prove a loop invariant inductive (k-induction)"
    )
    ...
    equiv = sub.add_parser(
        "prove-equivalence", help="prove bounded bisimulation between two V2 machines"
    )
    ...
    trust_cmd = sub.add_parser(
        "manage-trust", help="manage the authorized-reviewer key registry"
    )
    ...
    compose = sub.add_parser(
        "compose",
        help="compose reviewed V2 domains and prove the glue with OpenJML ESC",
    )
    ...
    compose.add_argument(
        "--out-dir", help="write the deterministic Java/JML sources here"
    )
    ...
    compose.add_argument(
        "--actors", help="comma-separated actor names for concurrent model preflight"
    )
    compose.add_argument(
        "--lang",
        choices=["java", "rust", "c", "cpp"],
    ...
    system.add_argument(
        "artifact", help="system architecture and component-input JSON path"
    )
    system.add_argument(
        "--out-dir",
        required=True,
    ...
    system.add_argument(
        "--mode",
        choices=["implement", "refactor", "correct"],
        default="implement",
    ...
    system.add_argument("--executable", default="formalspecgen", help=argparse.SUPPRESS)
    system.add_argument("--json", help="aggregate machine-readable verdict destination")
    unified = sub.add_parser(
        "unified-system", help="lower a unified architecture artifact"
    )
    ...
    analyze = sub.add_parser(
        "analyze-codebase", help="extract unreviewed architecture/domain candidates"
    )
    ...
    document.add_argument(
        "--out", required=True, help="Markdown documentation destination"
    )
...
def dispatch(
    ...
    if args.command == "draft":
        return command_draft(args, ui, store, state)
    if args.command == "implement":
        return command_implement(args, ui)
    if args.command == "verify":
        return command_verify(args, ui)
    if args.command == "verify-refactor":
        return command_verify_refactor(args, ui)
    if args.command == "optimize-algorithm":
        return command_optimize_algorithm(args, ui)
    if args.command == "discover-algorithms":
        return command_discover_algorithms(args, ui)
    if args.command == "assess-security":
        return command_assess_security(args, ui)
    if args.command == "security-inspect":
        return command_security_inspect(args, ui)
    if args.command == "security-exploit":
        return command_security_exploit(args, ui)
    if args.command == "remediate":
        return command_remediate(args, ui)
    if args.command == "correct-behavior":
        return command_correct_behavior(args, ui)
    if args.command == "verify-bisimulation":
        return command_verify_bisimulation(args, ui)
    if args.command == "inspect":
        return command_inspect(args, ui)
    if args.command == "apply-refactor":
        return command_apply_refactor(args, ui)
    if args.command == "architecture":
        return command_architecture(args, ui)
    if args.command == "design-system":
        return command_design_system(args, ui)
    if args.command == "validate-architecture":
        return command_validate_architecture(args, ui)
    if args.command == "domain":
        return command_domain(args, ui, store, state)
    if args.command == "validate-domain":
        return command_validate_domain(args, ui)
    if args.command == "promote-domain":
        return command_promote_domain(args, ui)
    if args.command == "sign-artifact":
        return command_sign_artifact(args, ui)
    if args.command == "manage-trust":
        return command_manage_trust(args, ui)
    ...
    if args.command == "compose":
        return command_compose(args, ui)
    if args.command == "reverify":
        return command_reverify(args, ui)
    if args.command == "system":
        return command_system(args, ui)
    if args.command == "unified-system":
        return command_unified_system(args, ui)
    if args.command == "analyze-codebase":
        return command_analyze_codebase(args, ui)
    if args.command == "document-code":
        return command_document_code(args, ui)
    return 2
...
def repl(
    ...
    while True:
        ...
        if line == "/help":
            ...
            continue
        if line == "/session":
            _write_json(state, None, ui.console)
            continue
        if line == "/reset":
            store.clear()
            state.clear()
...
def main(argv: list[str] | None = None) -> int:
    ...
    code = (
        dispatch(args, ui, store, state)
        if args.command
        else repl(parser, ui, store, state)
    )
    if argv is None:

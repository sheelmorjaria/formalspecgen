...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand('formalspecgen.generateDomainPlugin',
      () => generateDomainPlugin(architecture)),
  ...
  private async handle(message: { type: string; requirement?: string; domain?: string;
    ...
    if (message.type === 'ready') {
      ...
      if (this.state.domain === 'new') {
        const spec = await generateDomainPlugin(this.architecture, this.state.requirement);
        if (spec?.module_name) {
...
async function generateDomainPlugin(architecture: ArchitectureViewProvider,
                                    initialIdea?: string): Promise<DomainSpecArtifact | undefined> {
  const idea = initialIdea ?? await vscode.window.showInputBox({
  ...
  try {
    await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification,
      ...
      const elicited = await requestBackend(
        { action: 'elicit_domain_questions', idea, provider }, 'domain_questions');
      const questions = elicited.questions ?? [];
      const answers: Array<{ id: string; answer: string }> = [];
      for (const question of questions) {
        const answer = await vscode.window.showInputBox({
          title: `${question.category}: ${question.required ? 'required' : 'optional'}`,
          prompt: question.question, ignoreFocusOut: true
        });
        if (question.required && !answer?.trim()) {
          throw new Error(`Required clarification was not answered: ${question.question}`);
        }
        if (answer?.trim()) { answers.push({ id: question.id, answer: answer.trim() }); }
      }
      const result = await requestBackend(
        { action: 'compile_domain_spec', idea, questions, answers, provider },
        'domain_spec_result');
      ...
      const accept = await vscode.window.showWarningMessage(
        'The schema is validated, but operation semantics and TLA+ templates still require human review. Write the fail-closed scaffold to this workspace?',
        { modal: true }, 'Write scaffold');
      if (accept !== 'Write scaffold') { return; }
      await writeDomainScaffold(result);
      await architecture.populateFromDomain(result.spec, idea);
...
async function writeDomainScaffold(result: ProtocolEvent): Promise<void> {
  ...
  if (!root || !result.yaml || !result.spec?.module_name || !result.files) {
    throw new Error('Open a writable project workspace before generating a domain plugin.');
  }
  ...
  for (const relative of Object.keys(artifacts)) {
    ...
    try { await vscode.workspace.fs.stat(uri); throw new Error(`refusing to overwrite ${relative}`); }
  ...
  if (result.registration) {
    const registryUri = vscode.Uri.joinPath(root, 'pipeline', 'domains', 'registry.py');
    let registry: string;
    ...
    edit.replace(registryUri, new vscode.Range(0, 0, document.lineCount, 0), registry);
  }
  if (!await vscode.workspace.applyEdit(edit)) { throw new Error('VS Code rejected the scaffold write'); }

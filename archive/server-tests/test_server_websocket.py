...
class ServerWebSocketTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_capabilities_lints_discovery_and_unknown_action(self):
        ...
        self.assertEqual([item["type"] for item in socket.sent], [
            "protocol_info", "capabilities", "lint_result", "rust_lint_result", "pass_suggestions", "error"])
        self.assertEqual(socket.sent[0]["protocol_version"], server.PROTOCOL_VERSION)
        self.assertIn("domain_generation", socket.sent[0]["capabilities"])
        self.assertIn("bank_account", socket.sent[1]["tla_domains"])
        self.assertEqual(socket.sent[1]["domain_generation_protocol_version"], 2)
        self.assertIn("domain_plugin_generation", socket.sent[1]["features"])

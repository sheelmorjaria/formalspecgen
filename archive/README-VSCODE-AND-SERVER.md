...
FormalSpecGen is a terminal-first, human-in-the-loop verification tool for turning natural-language
requirements into reviewed, mechanically checked formal specifications and verified code. The VS
Code extension and FastAPI service remain optional clients over the same Python core.
...
Install the repository in editable mode while developing:
...
Running `formalspecgen` without arguments starts the interactive REPL. It supports multi-line-aware
terminal editing and persistent command history. Clarification questions and answers are checkpointed
in `.formalspecgen/session.json`, so an interrupted session can resume without repeating answers.
Secrets are never written to the session file.

```text
formalspecgen
> Design a bounded counter that never exceeds 1000.

/draft "Design a bounded counter that never exceeds 1000."
/verify runs/counter/.../Counter.java --mode check
/implement runs/counter/.../Counter.java --provider ollama
/architecture BankAccount.java --abstraction atomic_operations
/domain "A two-direction traffic-light controller with no simultaneous green lights"
/session
/reset
/quit
```

The same operations are available as non-interactive subcommands for scripts and CI:

```bash
formalspecgen draft "A counter starts at zero and never exceeds 1000" --provider ollama
formalspecgen verify Counter.java --mode esc --json verdict.json
formalspecgen implement Counter.java --provider ollama --json implementation-verdict.json
formalspecgen architecture BankAccount.java --emit-tla BankAccount.tla --json architecture.json
formalspecgen domain "An elevator controller" --project-root .
```

Exit status is zero only when the requested gate succeeds. `draft` establishes a checked JML
contract, `verify --mode esc` and `implement` can establish deductive proof, and `architecture`
reports bounded architectural evidence without claiming source refinement. The CLI calls Python
pipeline modules directly; it does not require FastAPI, WebSockets, or a running VS Code extension.

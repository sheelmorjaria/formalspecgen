...
external packages can add high-drift CLI schemas through the versioned command-plugin API.
...
### FormalKernel integration

FormalKernel now lives in its own repository and consumes FormalSpecGen as a
pinned external dependency through `formalspecgen.cli-command-plugin.v1`.
Kernel sources, release evidence, kernel-specific commands, and promotion
actions are intentionally not distributed by this repository.

The `verify-kernel` command and its evidence lattice are registered by the
FormalKernel plugin and are not distributed by this package.
